...
naive<- naiveBayes(y, data = trainingData)
prediction <- predict(naive, testData, type = "raw")
prediction <- round(prediction)

times = nrow(prediction)
pred_usable <- rep(1, times)
...
accuracy <- table(pred_usable, testData[[c]])
